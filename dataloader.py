...
def load_data_mat(mat_path, nr_fea=None, nr_class=None, normalize=False):
    '''
    ...
    data = data_mat['data']
    if nr_fea is None:
        nr_fea = data[0, 0].shape[1]
    if nr_class is None:
        all_labels = []
        for i in range(data.shape[0]):
            labels = list(data[i, 1].flatten()) + list(data[i, 2].flatten())  # partial + full
            all_labels.extend(labels)
        nr_class = int(np.max(all_labels))
    ...
    all_ins_fea = torch.from_numpy(all_ins_fea).float()
    bag_idx_of_ins = torch.from_numpy(bag_idx_of_ins).long()
    dummy_ins_lab = torch.from_numpy(dummy_ins_lab).long()
    bag_lab = torch.from_numpy(bag_lab).long()
    partial_bag_lab = torch.from_numpy(partial_bag_lab).float()
    partial_bag_lab_processed = torch.from_numpy(partial_bag_lab_processed).float()
...
def setup_scatter(Xs):
    ...
    i_ptr = torch.cat(
        [torch.tensor([0], device=device), i.bincount().cumsum(0)])  # recording the start position of each bag
    return x, i, i_ptr