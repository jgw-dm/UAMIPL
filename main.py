...
if args.ds.startswith("CRC"):
    mat_path = os.path.join(args.data_path, args.ds + '/' + args.ds + '.mat')
else:
...
all_ins_fea, bag_idx_of_ins, dummy_ins_lab, bag_lab, partial_bag_lab, partial_bag_lab_processed = load_data_mat(
    mat_path, None, None, normalize=args.normalize)
args.nr_fea = all_ins_fea.shape[1]
args.nr_class = partial_bag_lab.shape[1]
...
if __name__ == "__main__":
    ...
    for trial_i in range(num_trial):
        ...
        for fold_i in all_folds:
            ...
            model = UAMIPL.initialize_model(x_tr, cov_b, s_bag_tr, args)
            model.to(device)
            x_tr, x_te, y_ins_tr, s_bag_tr, y_bag_tr = \
                ...
                y_ins_tr.to(device).to(torch.float32), s_bag_tr.to(device).to(torch.float32), y_bag_tr.to(device).to(
                    torch.float32)
            ...
            model.fit(train_loader, num_bags_tr, weight_list=alpha_list)
